...
public class Monster : MonoBehaviour
{
	...
	private void Rotate()
	{
		...
		if(rotateTimer < 0f && InPath())
		{
			...
			if(gameObject.transform.position.x>5f){angl=180f;}
			if(-5f>gameObject.transform.position.x){angl = 0f;}
	...
	private bool InPath()
	{
		if(Near(gameObject.transform.position.y,0f,max_speed) && -5f-max_speed<gameObject.transform.position.x && gameObject.transform.position.x<5f+max_speed){return true;}
		return false;
	}
